...
public class DecimalFormat extends NumberFormat {
	...
	/*
	 * protected String curPosPrefix; protected String curPosSuffix; protected
	 * String curNegPrefix; protected String curNegSuffix;
	 */
	...
	public void applyPattern(String pattern) {
		...
		// curNegPrefix = String.valueOf(formatSymbols.getMinusSign()) +
		// curPosPrefix;
		// curNegSuffix = "";
		// }
		posPrefix = "";
	...
	public boolean equals(Object other) {
		...
		return super.equals(df) && Util.nullEquals(formatSymbols, df.formatSymbols)
				&& Util.nullEquals(posPrefix, df.posPrefix) && Util.nullEquals(negPrefix, df.negPrefix)
				&& Util.nullEquals(negSuffix, df.negSuffix) && Util.nullEquals(posSuffix, df.posSuffix)
				&& multiplier == df.multiplier && groupingSize == df.groupingSize && useExponential == df.useExponential
	...
	@Override
	public int hashCode() {
		return super.hashCode() * 2 + Util.nullHash(formatSymbols) * 3 + Util.nullHash(posPrefix) * 5
				+ Util.nullHash(negPrefix) * 7 + Util.nullHash(negSuffix) * 11 + Util.nullHash(posSuffix) * 13
				+ multiplier * 17 + groupingSize * 19 + (useExponential ? 23 : 0) + minExponentDigits * 29
	...
	public void setPositivePrefix(String posPrefix) {
		...
		// monetaryPrefix = (idx >= 0);
		// if (monetaryPrefix) {
		// useCurrencyCode = (idx < posPrefix.length() && posPrefix.charAt(idx +
		// 1) == CURRENCY_SYMBOL);
		// }
		// monetary = monetaryPrefix | monetarySuffix;
	...
	public void setPositiveSuffix(String posSuffix) {
		...
		// monetarySuffix = idx >= 0;
		// if (monetarySuffix) {
		// useCurrencyCode = (idx < posSuffix.length() && posSuffix.charAt(idx +
		// 1) == CURRENCY_SYMBOL);
		// }
		// monetary = monetaryPrefix | monetarySuffix;
	 ...
	 *            exponential value
	 * @param result
	 *            formatted exponential part will be append to it
	 */
	 ...
	 * @param result
	 *            where the formatted string goes
	 */
	...
	private void subformatFixed(double number, StringBuffer result, int minIntDigits) {
		...
		char decimal = formatSymbols.getDecimalSeparator(); // monetary ?
															// formatSymbols.getMonetaryDecimalSeparator()
	 ...
	 *            minimum integer digits
	 */
	private void subformatFixed(long number, StringBuffer result, int minIntDigits) {
